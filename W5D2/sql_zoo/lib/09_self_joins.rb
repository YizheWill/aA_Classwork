...
def lrt_stops
  ...
  execute(<<-SQL)
  select id, name from stops as s
  join routes as r on r.stop_id = s.id
...
def craiglockhart_and_tollcross
  ...
  from routes r
  join routes t
  on r.company = t.company and r.num = t.num
  join stops a
  on a.id = r.stop_id
  join stops b
  on b.id = t.stop_id
...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
  select s.name, b.company, b.num
  from routes a
  join routes b on a.company = b.company and a.num = b.num
  join stops s on s.id = b.stop_id
  join stops t on t.id = a.stop_id
  where t.name = 'Craiglockhart'
  group by s.name, b.company, b.num
